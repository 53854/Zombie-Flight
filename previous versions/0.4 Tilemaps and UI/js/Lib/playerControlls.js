...
function playerMove(){
    ...
    if (player.body.onFloor() && playerFlying == true){
...
for (var i = 0; i < player.children.length; i++){ //checks all existing player balloons
        if(player.getChildAt(i).body.onCeiling()){      //balloon collision checks
            console.log("called  top collide");
            player.body.velocity.y = 100;
        }
        else if (player.getChildAt(i).body.onWall()){
            console.log("called  side collide");
